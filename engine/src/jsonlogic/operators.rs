//! JSONLogic Standard Operators Implementation
...
pub fn eval_and(engine: &JsonLogicEngine, args: &[Rule], data: &Value) -> Result<Value, JsonLogicError> {
...
pub fn eval_or(engine: &JsonLogicEngine, args: &[Rule], data: &Value) -> Result<Value, JsonLogicError> {
    // Short-circuit evaluation
    for arg in args {
        let result = engine.evaluate(arg, data)?;
        if is_truthy(&result) {
...
pub fn eval_missing_some(args: Vec<Value>, data: &Value) -> Result<Value, JsonLogicError> {
    ...
    if found.len() >= min_required {
        ...
        let missing: Vec<Value> = keys.iter()
...
pub fn eval_var(args: &[Rule], data: &Value) -> Result<Value, JsonLogicError> {
    ...
    // First argument should be the path (as a primitive)
    ...
    // Default value is second argument (optional)
...
pub fn eval_if(engine: &JsonLogicEngine, args: &[Rule], data: &Value) -> Result<Value, JsonLogicError> {
    if args.is_empty() {
        return Ok(Value::Null);
    }
...
pub fn eval_map(engine: &JsonLogicEngine, args: &[Rule], data: &Value) -> Result<Value, JsonLogicError> {
    if args.len() < 2 {
        return Ok(Value::Array(vec![]));
    }
...
pub fn eval_filter(engine: &JsonLogicEngine, args: &[Rule], data: &Value) -> Result<Value, JsonLogicError> {
    if args.len() < 2 {
        return Ok(Value::Array(vec![]));
    }
...
pub fn eval_reduce(engine: &JsonLogicEngine, args: &[Rule], data: &Value) -> Result<Value, JsonLogicError> {
    if args.len() < 2 {
        return Ok(Value::Null);
    }
...
pub fn eval_all(engine: &JsonLogicEngine, args: &[Rule], data: &Value) -> Result<Value, JsonLogicError> {
    if args.len() < 2 {
        return Ok(Value::Bool(false));
    }
...
pub fn eval_none(engine: &JsonLogicEngine, args: &[Rule], data: &Value) -> Result<Value, JsonLogicError> {
    if args.len() < 2 {
        return Ok(Value::Bool(true));
    }
...
pub fn eval_some(engine: &JsonLogicEngine, args: &[Rule], data: &Value) -> Result<Value, JsonLogicError> {
    if args.len() < 2 {
        return Ok(Value::Bool(false));
    }
